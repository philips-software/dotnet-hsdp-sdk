...
public class HsdpIam : IHsdpIam
{
    ...
    public async Task<TokenMetadata> Introspect(IIamToken token)
    {
        ...
        var introspectResponse =
            await http.HttpRequestWithBasicAuth<IntrospectResponse>(requestContent, IntrospectPath, apiVersion);
    ...
    public async Task<HsdpUserInfo> GetUserInfo(IIamToken token)
    {
        ...
        var getUserInfoResponse =
            await http.HttpRequestWithBearerAuth<UserInfoResponse>(requestContent, UserInfoPath, HttpMethod.Get,
                apiVersion, token);
    ...
    private static IamToken CreateIamToken(TokenResponse tokenResponse)
    {
        return new IamToken(
            ...
            DateTime.UtcNow.AddMinutes(tokenResponse.expires_in),
            tokenResponse.token_type,
            tokenResponse.scopes,
            tokenResponse.id_token,
            tokenResponse.signed_token,
            tokenResponse.issued_token_type,
    ...
    private static TokenMetadata CreateTokenMetadata(IntrospectResponse introspectResponse)
    {
        return new TokenMetadata
        {
            ...
            Organizations = introspectResponse.organizations != null
                ? CreateOrganizations(introspectResponse.organizations)
                : null,
